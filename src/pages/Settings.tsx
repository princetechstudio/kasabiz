...
import { Bell, Building2, Check, Crown, Monitor, Moon, RefreshCcw, Save, Sun, UserRound } from "lucide-react";
...
export default function Settings() {
  ...
  const [profile, setProfile] = useState({
    ...
    email: session?.user.email ?? "prince@kasabiz.demo",
    phone: session?.user.phone ?? "024 555 0182",
  });
            ...
            <div className="mt-5 space-y-2.5">
              {([
                ...
                <button
                  key={option.plan}
                  onClick={() => {
                    if (data.plan === option.plan) return;
                    dispatch({ type: "PLAN_SET", plan: option.plan });
                    toast(`${option.plan} plan activated for frontend testing.`, "success");
                  }}
                  className={cx(
                    "w-full rounded-xl border px-3.5 py-3 text-left transition",
                    data.plan === option.plan ? "border-gold bg-white/12" : "border-white/15 hover:border-gold/60 hover:bg-white/8"
                  )}
                >
                  <span className="flex items-center justify-between gap-3">
                    <span>
                      <span className="block text-sm font-bold text-white">{option.plan}</span>
                      <span className="block text-xs text-white/60 mt-0.5">{option.detail}</span>
                    </span>
                    <span className="text-xs font-bold text-gold">{data.plan === option.plan ? "Current" : option.price}</span>
                  </span>
                </button>
              ))}
            </div>
            <Button variant="gold" className="mt-5" onClick={() => nav("/pricing")}>View full comparison</Button>
          </Card>
        </div>
      )}
