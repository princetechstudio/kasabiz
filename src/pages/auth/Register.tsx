...
export default function Register() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast("Akwaaba! Your business is set up.", "success");
  ...
  return (
    <AuthFrame>
      <h1 className="font-display font-extrabold text-[28px] text-ink">Create your free account</h1>
      ...
      <form onSubmit={submit} className="mt-6 space-y-4" noValidate>
        ...
        <Field label="Password" error={errors.password} hint={!errors.password ? "6+ characters. Mix letters and numbers." : undefined}>
        ...
        <ul className="flex flex-wrap justify-center gap-x-5 gap-y-1.5 pt-1">
          {["Free forever plan", "No card required", "Set up in minutes"].map((t) => (
            <li key={t} className="flex items-center gap-1.5 text-xs font-semibold text-sub"><Check className="size-3.5 text-ok" /> {t}</li>
          ))}
        </ul>
      </form>

      <div className="my-5 flex items-center gap-3 text-[11px] font-bold uppercase tracking-widest text-faint">
        <span className="h-px flex-1 bg-line" /> or <span className="h-px flex-1 bg-line" />
      </div>
      <Button variant="secondary" size="lg" className="w-full" onClick={async () => {
        setAuthError(null);
        try {
          await authService.loginWithGoogle();
        } catch (error) {
          const message = error instanceof Error ? error.message : "Google signup is unavailable.";
          setAuthError(message);
          toast(message, "error");
        }
      }}>
